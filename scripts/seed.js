...
async function fetchRecipes(type, number = 20) {
  ...
  if (!res.ok) throw new Error(`Spoonacular error: ${res.status} ${res.statusText}`)
...
async function seed() {
  ...
  const first12 = await prisma.recipe.findMany({ take: 12, orderBy: { id: 'asc' } })
  for (const r of first12) {
    await prisma.recipe.update({ where: { id: r.id }, data: { featured: true } })
  }

  console.log(`Done — ${total} recipes seeded, ${first12.length} marked as featured.`)
